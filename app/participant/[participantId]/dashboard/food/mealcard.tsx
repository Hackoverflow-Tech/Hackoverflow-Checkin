...
export default function MealCard({
  ...
  // Redemption time is tracked locally only — DB stores a simple boolean now
  const [availedAt, setAvailedAt] = useState<string | null>(null);
  ...
  useEffect(() => {
    const checkTimes = () => {
      ...
      if (isAvailed && availedAt) {
        ...
        const oneHourLater = new Date(redeemedTime.getTime() + 60 * 60 * 1000);
  ...
  const handleRedeem = async () => {
    if (
      !confirm(
        `Are you sure you want to redeem ${label} now? The token will only be visible for 1 hour!`
      )
    ) {
      return;
    }
    ...
    if (result.success) {
      ...
      const errorMsg =
        (result as any).error || "Failed to redeem meal. Please try again.";
          ...
          <div className="flex items-center gap-3">
            <div
              className={`p-3 rounded-xl ${
                isAvailed ? "bg-green-500/10 text-green-500" : "bg-white/5 text-gray-400"
              }`}
            >
              🍔
            </div>
        ...
        <div className="mt-auto pt-4 border-t border-white/5 flex flex-col gap-3">
          ...
          {isAvailed && !isTokenActive && (
            <div className="w-full py-2 text-center text-xs font-medium text-gray-400 bg-white/5 rounded-lg mt-2 border border-white/5">
              {availedAt ? `Redeemed at ${displayTime.split(", ")[1] || "Counter"}` : "Consumed"}
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
          <div className="bg-[#111111] border border-green-500/30 rounded-2xl p-8 max-w-sm w-full text-center shadow-[0_0_30px_rgba(34,197,94,0.15)] animate-in fade-in zoom-in duration-200">
            ...
            <p className="text-gray-400 text-sm mb-6">
              Show this screen to the counter staff immediately.
            </p>
            ...
            <div className="bg-black/50 p-4 rounded-xl border border-white/10 mb-6 space-y-2 relative overflow-hidden">
              ...
              <p className="text-3xl font-mono font-bold text-[#FCB216] tracking-widest">
                {uniqueCode}
              </p>
            ...
            <p className="text-[10px] text-gray-600 mt-4">
              Token will automatically expire 1 hour after redemption.
            </p>
          </div>
        </div>
      )}
