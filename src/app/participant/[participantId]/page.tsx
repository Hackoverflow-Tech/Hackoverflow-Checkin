...
export default function CheckInPage() {
  ...
  async function handleCheckIn() {
    ...
    try {
      ...
      if (data.success) {
        setDone(true);
        setTimeout(() => router.push(`/directions/${participantId}`), 1500);
      } else {
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .orb-1 {
          ...
          background: radial-gradient(circle, rgba(252,178,22,0.1), transparent 70%);
          top: -150px; left: -150px;
        }
        .orb-2 {
          width: 600px; height: 600px;
          background: radial-gradient(circle, rgba(217,27,87,0.08), transparent 70%);
          bottom: -200px; right: -150px;
          animation-delay: -2.5s;
        }
        .orb-3 {
          width: 400px; height: 400px;
          background: radial-gradient(circle, rgba(232,93,36,0.07), transparent 70%);
          top: 50%; left: 50%;
        ...
        .info-row:nth-child(2) { animation-delay: 0.1s; }
        .info-row:nth-child(3) { animation-delay: 0.2s; }
        .info-row:nth-child(4) { animation-delay: 0.3s; }
        ...
        .team-badge {
          ...
          border-radius: 50px;
          background: rgba(99,32,95,0.3);
          border: 1px solid rgba(99,32,95,0.6);
          color: #D91B57;
          letter-spacing: 1px;
          white-space: nowrap;
        }
        ...
        .checkin-btn:hover:not(:disabled) {
          ...
          box-shadow: 0 12px 30px rgba(232,93,36,0.4);
        }
        ...
        .error-box {
          padding: 0.75rem 1rem;
          background: rgba(217,27,87,0.1);
          border: 1px solid rgba(217,27,87,0.3);
          border-radius: 12px;
          color: #D91B57;
          font-size: 0.85rem;
          text-align: center;
      ...
      <div className="page">
        <div className="card">
          ...
          <div className="logo-row">
            <span className="logo-badge">⚡ Hackoverflow 4.0</span>
          </div>
          ...
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div className="skeleton" style={{ height: '56px' }} />
              <div className="skeleton" style={{ height: '56px' }} />
              <div className="skeleton" style={{ height: '52px', marginTop: '0.5rem' }} />
            </div>
          ) : error ? (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.85rem', marginTop: '0.5rem' }}>
                Please contact the event staff for help.
              </p>
            </div>
          ) : participant ? (
            <>
              ...
              <div className="info-grid">
                {participant.role && (
                  <div className="info-row">
                    ...
                    <div className="info-content">
                      ...
                      <div className="info-value">{participant.role}</div>
                    </div>
                  </div>
                )}
                ...
                {participant.institute && (
                  <div className="info-row">
                    ...
                    <div className="info-content">
                      <div className="info-label">Institute</div>
                      <div className="info-value">{participant.institute}</div>
                    </div>
                  </div>
                )}
                ...
                {(participant.teamName || participant.teamId) && (
                  <div className="info-row">
                    ...
                    <div className="info-content">
                      ...
                      <div className="info-value">{participant.teamName || '—'}</div>
                    </div>
                    {participant.teamId && (
                      <span className="team-badge">{participant.teamId}</span>
                    )}
                  </div>
                )}
                ...
                {participant.labAllotted && (
                  <div className="info-row">
                    <div className="info-icon">🔬</div>
                    <div className="info-content">
                      <div className="info-label">Lab Allotted</div>
                      <div className="info-value">{participant.labAllotted}</div>
                    </div>
                  </div>
                )}
              </div>
              ...
              {done ? (
                <>
                  <div className="already-badge">
                    ✅ College check-in already recorded
                  </div>
                  <button
                    className="already-btn"
                    onClick={() => router.push(`/directions/${participantId}`)}
                  >
                    Go to Directions →
                  </button>
                </>
              ) : checking ? (
                <div className="success-state" style={{ marginTop: '0.5rem' }}>
                  <div className="success-icon">✅</div>
                  <p className="success-title">Checked In!</p>
                  <p className="success-sub">Redirecting to directions…</p>
                </div>
              ) : (
                <button
